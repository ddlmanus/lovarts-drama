        ...
        <div class="settings-head">
          <div class="settings-brand">
            <div class="settings-brand-mark">
              <img v-if="showBrandImage" :src="brandLogo" alt="Lovarts短剧平台" class="settings-brand-logo" @error="showBrandImage = false" />
            ...
            <div class="settings-brand-copy">
              ...
              <div class="settings-brand-name">Lovarts短剧平台</div>
            </div>
          </div>
    ...
    <div v-if="cfgDialog" class="overlay" @click.self="cfgDialog = false">
      <form class="modal card config-modal" @submit.prevent="saveCfg">
        ...
        <label class="field">
          ...
          <input v-model="cfgForm.name" class="input" placeholder="如 Lovarts短剧平台默认图像服务" />
        </label>
      ...
      <form class="modal card config-modal" @submit.prevent="applyHuobaoPreset">
        <div class="config-modal-head">
          <div>
            ...
            <h2 class="modal-title">Lovarts短剧平台一键配置</h2>
            <div class="modal-note">输入Lovarts短剧平台开放平台 API Key 后，文本、图片、视频、音频都会使用Lovarts短剧平台推荐链路，并同时初始化 5 个 Agent 的默认模型。</div>
          </div>
          <span class="tag tag-success">推荐</span>
        </div>
        <div class="huobao-grid">
          <label class="field">
            <span class="field-label">Lovarts短剧平台 API Key <span class="dim">(文本 / 图片 / 视频 / 音频统一使用)</span></span>
            <input v-model="huobaoForm.apiKey" class="input" type="password" placeholder="用于Lovarts短剧平台文本、图片、视频、音频服务" />
            <span class="field-hint">Lovarts短剧平台 API Key 可在Lovarts短剧平台开放平台获取</span>
          </label>
        </div>
...
const providerPresets = {
  text: {
    ...
    chatfire: { label: 'Lovarts短剧平台文本', baseUrl: 'https://api.chatfire.site', models: ['gemini-3.1-pro-preview', 'gemini-3-flash-preview', 'gemini-3-pro-preview-thinking'] },
    ...
    gemini: { label: 'Google Gemini 推荐', baseUrl: 'https://generativelanguage.googleapis.com', models: ['gemini-3.1-pro-preview', 'gemini-3-flash-preview', 'gemini-2.5-flash'] },
  ...
  image: {
    ...
    chatfire: { label: 'Lovarts短剧平台图片', baseUrl: 'https://api.chatfire.site', models: ['doubao-seedream-4-5-251128'] },
    gemini: { label: 'Google Gemini 图片', baseUrl: 'https://generativelanguage.googleapis.com', models: ['gemini-3.1-flash-image-preview', 'gemini-3-pro-image-preview', 'gemini-2.5-flash-image'] },
    volcengine: { label: '火山推荐', baseUrl: 'https://ark.cn-beijing.volces.com', models: ['doubao-seedream-4-0-250828'] },
  },
  video: {
    apimart: { label: 'APIMart 视频', baseUrl: 'https://api.apimart.ai', models: ['skyreels-v4-fast', 'skyreels-v4-std', 'kling-v3-omni', 'viduq3-pro', 'viduq3-turbo', 'wan2.7-videoedit', 'Omni-Flash-Ext', 'grok-imagine-1.0-video-apimart', 'happyhorse-1.0', 'doubao-seedance-2.0', 'doubao-seedance-2.0-fast'] },
    volcengine: { label: 'Lovarts短剧平台视频', baseUrl: 'https://api.chatfire.site/volcengine', models: ['doubao-seedance-1-5-pro-251215'] },
  ...
  audio: {
    chatfire: { label: 'Lovarts短剧平台音频', baseUrl: 'https://api.chatfire.site/minimax', models: ['speech-2.8-hd'] },
    minimax: { label: 'MiniMax 音频', baseUrl: 'https://api.chatfire.site/minimax', models: ['speech-2.8-hd'] },
  },
}
const huobaoPresetCards = [
  { serviceType: 'text', label: '文本', provider: 'Lovarts短剧平台', baseUrl: 'https://api.chatfire.site', model: 'gemini-3.1-pro-preview', priority: 100 },
  { serviceType: 'image', label: '图片', provider: 'Lovarts短剧平台', baseUrl: 'https://api.chatfire.site', model: 'doubao-seedream-4-5-251128', priority: 99 },
  { serviceType: 'video', label: '视频', provider: 'Lovarts短剧平台', baseUrl: 'https://api.chatfire.site/volcengine', model: 'doubao-seedance-1-5-pro-251215', priority: 98 },
  { serviceType: 'audio', label: '音频', provider: 'Lovarts短剧平台', baseUrl: 'https://api.chatfire.site/minimax', model: 'speech-2.8-hd', priority: 97 },
]
...
async function applyHuobaoPreset() {
  ...
  try {
    ...
    toast.success('Lovarts短剧平台推荐配置与默认 Agent LLM 已写入')
  } catch (e) {
    toast.error(e.message)
  }
}
