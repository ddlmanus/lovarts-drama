...
import { ensureOwnedEpisode, requestUserId } from '../utils/dramaAccess.js'
...
app.post('/episodes/:id/merge', async (c) => {
  ...
  try {
    ...
    const mergeId = await mergeEpisodeVideos(episodeId, ep.dramaId, requestUserId(c))
