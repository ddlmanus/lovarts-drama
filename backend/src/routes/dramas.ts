...
app.get('/stats', async (c) => {
  const all = await db.select().from(schema.dramas).where(isNull(schema.dramas.deletedAt)).execute()
...
app.put('/:id', async (c) => {
  ...
  await db.update(schema.dramas).set(updates).where(eq(schema.dramas.id, id)).execute()
  return success(c)
})
