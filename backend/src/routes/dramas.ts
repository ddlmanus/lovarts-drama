...
app.get('/', async (c) => {
  ...
  let query = db.select().from(schema.dramas).where(eq(schema.dramas.userId, userId))

  const allRows = await query.orderBy(desc(schema.dramas.updatedAt))
  let filtered = allRows.filter(d => !d.deletedAt)
...
app.get('/stats', async (c) => {
  const userId = requestUserId(c)
  const all = (await db.select().from(schema.dramas).where(eq(schema.dramas.userId, userId)).execute())
...
app.put('/:id', async (c) => {
  ...
  await db.update(schema.dramas).set(updates).where(and(eq(schema.dramas.id, id), eq(schema.dramas.userId, userId))).execute()
  return success(c)
})
...
app.put('/:id/characters', async (c) => {
  ...
  for (const char of chars) {
    if (char.id) {
      await db.update(schema.characters).set({ ...char, updatedAt: ts, updatedBy: userId }).where(and(eq(schema.characters.id, char.id), eq(schema.characters.userId, userId))).execute()
    } else {
      await db.insert(schema.characters).values({ ...char, userId, dramaId, createdBy: userId, createdAt: ts, updatedBy: userId, updatedAt: ts }).execute()
    }
  }
  return success(c)
})
...
app.put('/:id/episodes', async (c) => {
  ...
  for (const ep of episodes) {
    if (ep.id) {
      await db.update(schema.episodes).set({ ...ep, updatedAt: ts, updatedBy: userId }).where(and(eq(schema.episodes.id, ep.id), eq(schema.episodes.userId, userId))).execute()
    } else {
      await db.insert(schema.episodes).values({
        ...ep,
        userId,
