...
app.post('/sync', async (c) => {
  ...
  if (rows.length === 0) {
    return badRequest(c, 'No active Huobao/MiniMax audio config found')
  }

  const config = rows[0]
  if (!config.apiKey) {
    return badRequest(c, 'Audio API key not configured')
  }
  ...
  if (!resp.ok) {
    return badRequest(c, `Audio API error: ${resp.status}`)
  }
