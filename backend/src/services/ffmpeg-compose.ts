...
export async function composeStoryboard(storyboardId: number): Promise<string> {
  const [sb] = await db.select().from(schema.storyboards).where(eq(schema.storyboards.id, storyboardId)).execute()
      ...
      if (!audioPath) {
        ...
        if (pureDialogue) {
          ...
          const ttsPath = await generateTTS({
            ...
            relatedTaskId: storyboardId,
            taskType: 'storyboard_compose_tts',
          })
          audioPath = toAbsPath(ttsPath)
          db.update(schema.storyboards).set({ ttsAudioUrl: ttsPath, updatedAt: now() })
            .where(eq(schema.storyboards.id, storyboardId)).execute()
        }
      }
    }
