...
async function linkCharToEpisode(episodeId: number, characterId: number) {
  ...
  const existing = await db.select().from(schema.episodeCharacters)
    ...
    .execute()
  if (!existing.length) {
    await db.insert(schema.episodeCharacters).values({ episodeId, characterId, createdAt: ts }).execute()
  }
}
...
async function linkSceneToEpisode(episodeId: number, sceneId: number) {
  ...
  const existing = await db.select().from(schema.episodeScenes)
    ...
    .execute()
  if (!existing.length) {
    await db.insert(schema.episodeScenes).values({ episodeId, sceneId, createdAt: ts }).execute()
  }
}
...
export function createExtractTools(episodeId: number, dramaId: number, taskId?: string) {
  ...
  const readScriptForExtraction = createTool({
    ...
    execute: async () => {
      updateAgentTask(taskId, {
        step: 'read_script_for_extraction',
        message: '正在读取剧本内容...',
        progress: 12,
      })
  ...
  const readExistingCharacters = createTool({
    ...
    execute: async () => {
      ...
      updateAgentTask(taskId, {
        step: 'characters_loaded',
        message: `已读取 ${payload.count} 个项目角色，正在分析场景...`,
        progress: 42,
        details: { projectCharacters: payload.count, episodeCharacters: payload.current_episode_characters.length },
      })
      return payload
    },
  })
  ...
  const readExistingScenes = createTool({
    ...
    execute: async () => {
      ...
      updateAgentTask(taskId, {
        step: 'scenes_loaded',
        message: `已读取 ${payload.count} 个项目场景，正在让模型提取角色和场景...`,
        progress: 62,
        details: { projectScenes: payload.count, episodeScenes: payload.current_episode_scenes.length },
      })
      return payload
    },
  })
    ...
    execute: async ({ characters }) => {
      ...
      for (const link of currentLinks) {
        if (!savedIds.includes(link.characterId)) {
          await db.delete(schema.episodeCharacters).where(eq(schema.episodeCharacters.id, link.id)).execute()
        }
      }
      ...
      updateAgentTask(taskId, {
        step: 'characters_saved',
        message: `角色保存完成：新增 ${results.created}，合并 ${results.merged}`,
        progress: 86,
        details: results,
      })
      return payload
  ...
  const saveDedupScenes = createTool({
    ...
    execute: async ({ scenes }) => {
      ...
      updateAgentTask(taskId, {
        step: 'scenes_saved',
        message: `场景保存完成：新增 ${results.created}，复用 ${results.reused}`,
        progress: 96,
        details: results,
      })
      return payload
